...
const FormGeneration: FunctionComponent<Props> = ({
    ...
    const createCustomerCategory = useMutation({
        mutationFn: data => Api.partner.insertCustomerCategory(data),
        onSuccess: () => {
            load();
            toast.success(
    ...
    const updateCustomerCategory = useMutation({
        mutationFn: data => Api.partner.updateCustomerCategory(id, data),
        onSuccess: () => {
            load();
            toast.success(
                'Thank you! Your request has been successfully sent.',
    ...
    const onSubmitHandler: SubmitHandler<any> = async data => {
        ...
        if (!!id) {
            await updateCustomerCategory.mutateAsync(dataForm);
        } else {
            await createCustomerCategory.mutateAsync(dataForm);
        }
    };
    ...
    const handleNavigateToEdit = useCallback(
        (itemId: number) => {
            navigate(`/partner/customer/category/${itemId}`);
        },
        [navigate],
    );
