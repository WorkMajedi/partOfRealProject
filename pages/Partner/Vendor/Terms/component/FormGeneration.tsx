...
const FormGeneration: FunctionComponent<Props> = ({
    ...
    const createVendorTerm = useMutation({
        mutationFn: data => Api.partner.insertVendorTerm(data),
        onSuccess: () => {
            load();
            reset(listData);
            toast.success(
    ...
    const updateVendorTerm = useMutation({
        mutationFn: data => Api.partner.updateVendorTerm(id, data),
        onSuccess: () => {
            load();
            reset(listData);
            toast.success(
    ...
    const onSubmitHandler: SubmitHandler<any> = async data => {
        ...
        if (!!id) {
            await updateVendorTerm.mutateAsync(dataForm);
        } else {
            await createVendorTerm.mutateAsync(dataForm);
        }
    };
    ...
    const handleNavigateToEdit = useCallback(
        (itemId: number) => {
            navigate(`/partner/vendor/term/${itemId}`);
        },
        [navigate],
    );
