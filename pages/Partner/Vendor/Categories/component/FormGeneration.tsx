...
const FormGeneration: FunctionComponent<Props> = ({
    ...
    const createVendorCategory = useMutation({
        mutationFn: data => Api.partner.insertVendorCategory(data),
        onSuccess: () => {
            load();
            toast.success(
    ...
    const updateVendorCategory = useMutation({
        mutationFn: data => Api.partner.updateVendorCategory(id, data),
        onSuccess: () => {
            load();
            toast.success(
                'Thank you! Your request has been successfully sent.',
    ...
    const onSubmitHandler: SubmitHandler<any> = async data => {
        ...
        if (!!id) {
            await updateVendorCategory.mutateAsync(dataForm);
        } else {
            await createVendorCategory.mutateAsync(dataForm);
        }
    };
    ...
    const handleNavigateToEdit = useCallback(
        (itemId: number) => {
            navigate(`/partner/vendor/category/${itemId}`);
        },
        [navigate],
    );
