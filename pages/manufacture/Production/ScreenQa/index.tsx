...
const ProductionQA: FunctionComponent = ({ screenDesign }: any) => {
    ...
    const { isLoading: isLoadingProduction } = useQuery<
        ...
        AxiosError
    >({
        queryKey: ['production-qa', production_code],
        ...
        cacheTime: 0,
        onSuccess: ({ data }) => {
            const DefaultVal = normalized.production.getQa(data);
    ...
    const { isLoading: isLoadingFormulaTemplate } = useQuery<
        ...
        AxiosError
    >({
        queryKey: ['production-qa', paramsId],
