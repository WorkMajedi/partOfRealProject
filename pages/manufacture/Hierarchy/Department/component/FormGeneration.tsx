...
const FormGeneration: FunctionComponent<Props> = ({
    ...
    const createDepartment = useMutation({
        mutationFn: data => Api.manufacture.insertDepartment(data),
        onSuccess: () => {
            load();
            toast.success(
    ...
    const updateDepartment = useMutation({
        mutationFn: data => Api.manufacture.updateDepartment(id, data),
        onSuccess: () => {
            load();
            toast.success(
                'Thank you! Your request has been successfully sent.',
    ...
    const onSubmitHandler: SubmitHandler<any> = async data => {
        ...
        if (!!id) {
            await updateDepartment.mutateAsync(dataForm);
        } else {
            await createDepartment.mutateAsync(dataForm);
        }
    };
    ...
    const handleNavigateToEdit = useCallback(
        (itemId: number) => {
            navigate(`/manufacture/hierarchy/department/${itemId}`);
        },
        [navigate],
    );
