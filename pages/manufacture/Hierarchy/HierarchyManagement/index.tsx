...
export default function HierarchyManagementScreen({ screenDesign }: any) {
    const { id } = useParams();
    ...
    const { isLoading } = useQuery<
        ...
        AxiosError
    >({
        queryKey: ['attachDtD', id],
        queryFn: () => Api.manufacture.getHierarchyManagementDetails(id),
        enabled: !!id,
        ...
        onSuccess: ({ data }) => {
            setDefaultValues({
                ...data,
                division: data?.code,
                division_description: data?.description,
            });
        },
    });
